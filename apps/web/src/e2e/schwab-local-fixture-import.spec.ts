...
test('local Schwab CSV fixture completes the visible import and sync workflow', async ({ page }) => {
  ...
  await page.getByLabel('选择结单文件').setInputFiles(schwabCsvFixture);
  ...
  await expect(page.getByText(/本次检测到|本次未发现新的缺失区间/, { exact: false })).toBeVisible({ timeout: 90_000 });

  const startSync = page.getByRole('button', { name: '立即同步' });
  if (await startSync.isEnabled()) {
    await startSync.click();
    await expect(page.getByText('同步状态：', { exact: false })).toBeVisible();
  }
  await expect(page.getByTestId('market-sync-pending-count')).toHaveText('0', { timeout: 360_000 });
