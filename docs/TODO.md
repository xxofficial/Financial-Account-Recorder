...
以下条目来自多角色动态体验评审。`待动态复核` 只表示已有静态线索，不能作为已确认缺陷；证据、角色结论和复测状态见 [动态浏览器体验评审报告](./reports/dynamic-browser-review-2026-07-16.md)。

| 编号 | 优先级 | 状态 | 待复核项 |
| :--- | :--- | :--- | :--- |
| UX-01 | P1 | 已实现待动态复核 | 导入结单后自动检测、入队并引导历史行情同步。 |
| UX-03 | P1 | 已实现待动态复核 | 个股详情的区间累计盈亏、期初市值、分红与独立税费勾稽。 |
| UX-04 | P1 | 已实现待动态复核 | 个股详情中利息、税费及负数其他流水的现金流符号与颜色。 |
| UX-05 | P1 | 已实现待动态复核 | 已结束历史区间的盈亏排行使用区间截止日价格。 |
| UX-06 | P2 | 已实现待动态复核 | 空账本/空持仓状态提供导入结单、手动记账等下一步入口。 |
| UX-07 | P2 | 已实现待动态复核 | 导入前显示目标账本、目标平台、密码来源和导入范围。 |
| UX-08 | P2 | 已实现待动态复核 | 候选支持逐条排除，并展示候选、跳过、警告及类型汇总。 |
| UX-10 | P2 | 已实现待动态复核 | 行情同步完成、失败、待重试与暂不支持均有明确摘要。 |
| UX-13 | P2 | 已实现待动态复核 | 具备证券代码的流水统一显示代码与名称。 |
| UX-15 | P3 | 已实现待动态复核 | 批量删除确认框提供先导出备份操作。 |
| UX-16 | P1 | 已实现待动态复核 | 个人/合资账本创建、切换、隔离及 Android 对齐的收益分摊视角。 |

## 本轮新增待复核项

| 编号 | 优先级 | 状态 | 待复核项 |
| :--- | :--- | :--- | :--- |
| UX-17 | P2 | 已实现待动态复核 | 导入入口、文件选择器和导入结果文案使用“已适配的 CSV / 文本 PDF 结单”，平台适配由解析器能力决定。 |
| UX-18 | P2 | 已实现待动态复核 | 页面提示已统一改为面向普通用户的可行动说明。 |
| UX-19 | P2 | 已实现待动态复核 | 设置页、行情缓存、结单导入、备份恢复、邮箱同步和账本管理使用统一错误分类。 |
